...
export class LoginComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.customerService.login(customer).subscribe((data) => {
      if (data != null) {
        ...
        this.authService.setLastName(data.lastName); // Fixed typo: 'setLasttName' to 'setLastName'
  ...
  redirectToSignup() {
    this.router.navigateByUrl('/register'); // Add the appropriate route for signup
  }
}